...
var printBoard = function () { 
    for (let i = 0; i < 9; i++){ //row
        for (let j = 0; j < 9; j++){ //col
            ...
            //var cellId = `r${i + 1}${j + 1}`; // identifies the cell id 
            var cell = document.getElementById(cellId); // puts the value into the cell 
            cell.textContent = sol[i][j] !== null ? sol[i][j]: ""; 
        }
    }
};
...
var loadNewPuzzle = function() {
    ...
    // use this end point to get puzzle  
    request.open("GET", "https://sudoku-api.vercel.app/api/dosuku");
    request.onload = function() {
        var data = JSON.parse(this.response);
        console.log(data);
        sol = data.newboard.grids[0].value; // value is the key 
        // think of it like a list grids[0]
        console.log(sol); 
        printBoard(); 
    }
    request.send();
